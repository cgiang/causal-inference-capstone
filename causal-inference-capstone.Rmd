...
plot(pc_data, main = "Pcalg Causal Graph")
```
...
mod12 <- lm(num_partner~householdcare, data = data)
summary(mod12)

mod13 <- lm(num_partner~traveling, data = data)
summary(mod13)

mod14 <- lm(num_partner~employ_partner, data = data)
summary(mod14)
...
```{r}
mod16 <- lm(life_satis~sleeping+num_partner, data = data)
summary(mod16)
...
> From model 16, controlling for num_partner makes sleeping and life satisfaction a significant association. Therefore, num_partner must be a collider.
...
```{r}
time_use$num_partner = as.factor(time_use$num_partner)
...
```{r}
ggplot(time_use, aes(x=sleeping_time, y=life_satis, fill=partner_employ)) + 
  ...
  ylab("Life satisfaction level") + xlab("Sleeping time (minutes)")
```



```{r, fig.width=6,fig.height=4}
ggplot(time_use, aes(x=traveling_time, y=partner_income, fill=num_partner)) + 
  geom_boxplot(alpha=0.5) +
  scale_fill_discrete(name = "Partner's Employment", labels = c("Spouse present", "Unmarried partner")) + 
  ggtitle("Sleeping time vs Life satisfaction") +
  ylab("Partner's weekly income (dollars)") + xlab("Traveling (minutes)")
```
...
```{r}
ps_mod = lm(sleeping_time ~ num_partner + partner_income, data = time_use)
...
```{r}
msm_fit <- svyglm(
    life_satis ~ sleeping_time*num_partner,
    data = time_use,
    design = design
)
summary(msm_fit)
...
```{r}
# Comparison - OK
summary(lm(life_satis ~ sleeping_time + num_partner, data = time_use, weights = time_use$ip_weight))
```
...
sensitivity_analysis_time <- function(.data, model_A, model_Y, assoc_A, assoc_Y) {
    ...
    updated_model <- lm(life_satis ~ sleeping_time + partner_income + num_partner + U, data = .data)
...
mod_A <- lm(sleeping_time ~ partner_income + num_partner, data = time_use)
mod_Y <- lm(life_satis ~ sleeping_time + partner_income + num_partner, data = time_use)
