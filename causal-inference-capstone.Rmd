...
data$SPEARNWEEK = as.numeric(data$SPEARNWEEK)
data <- data %>%
  filter(!is.na(WBLADDER)) %>%
  filter(WBLADDER > -1 & WBLADDER < 11) %>%
  filter(SPEDUC < 44) %>%
  filter(SPEMPSTAT < 7) %>%
  filter(SPEARNWEEK < 99999)
```
...
```{r}
ggplot(data, aes(x = SPEARNWEEK, y = Sleeping_time)) +
    geom_point() +
    geom_smooth(se = FALSE, color = "blue") +
...
```{r}
ps_mod = lm(Sleeping_time ~ SPOUSEPRES + SPEARNWEEK, data = data)

# estimation of denominator of ip weights
p <- predict(ps_mod, type = "response")
ps <-
  dnorm(data$Sleeping_time,
        p,
...
```{r}
msm_fit <- svyglm(
    WBLADDER ~ Sleeping_time*SPOUSEPRES,
    data = data,
    design = design
)
summary(msm_fit)
...
```{r}
# Comparison
summary(lm(WBLADDER ~ Sleeping_time + SPOUSEPRES, data = data))
```
...
sensitivity_analysis_time <- function(.data, model_A, model_Y, assoc_A, assoc_Y) {
    ...
    updated_model <- lm(WBLADDER ~ Sleeping_time + SPEARNWEEK + SPOUSEPRES + U, data = .data)
...
mod_A <- lm(Sleeping_time ~ SPEARNWEEK + SPOUSEPRES, data = data)
mod_Y <- lm(WBLADDER ~ Sleeping_time + SPEARNWEEK + SPOUSEPRES, data = data)
...
U_A_assocs <- seq(from = 50, to = 100, by = 10)
U_Y_assocs <- seq(from = 0.5, to = 1.6, by = 0.1)
...
ggplot(sens_data, aes(x = U_A, y = estimate)) +
    ...
    geom_line() +
    geom_hline(aes(yintercept = coefficients(mod_Y)["Sleeping_time"]), color = "blue", lty = "dashed") +
